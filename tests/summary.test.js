...
test('сводка: сколько можно, ближайший срок, лестница', () => {
  ...
  assert.deepEqual(summaryLines(me, programs, today), [
    'По условиям подходишь к 2 программам. Ближайший срок — 9 октября 2026, Программа b.',
    'Наберёшь IELTS 6.5 — будешь подходить ещё к 1 программе.',
  ]);
});
...
test('сводка отделяет программы без чисел от списка дел', () => {
  ...
  assert.ok(lines.some((l) => /^По условиям подходишь к 1 программе/.test(l)), lines.join(' | '));
});
