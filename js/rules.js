...
export function checkGpa(profile, rule, ctx) {
  ...
  if (mine < need) {
    ...
    // порогом. Отказывать по справочному числу нельзя.
    if (rule.advisory === true) {
      return r('unknown', `Твой балл — ${mine}%, у программы ориентир ${need}%. Это не порог: числом программа описывает своих поступивших, а не отсекает`);
    }
