...
// Годен ли ответ модели к показу человеку. Бесплатные модели с
// рассуждением иногда отдают в ответ свой черновик («We need to produce
// explanation…»), причём без тегов, и вырезать его нельзя. Поэтому ответ
// принимается, только если он начинается с первого заголовка и написан
// по-русски; иначе функция идёт к следующей модели.
export function isUsableAnswer(text) {
  if (typeof text !== 'string') return false;
  const t = text.trim();
  if (t.length < 80 || t.length > 3000) return false;
  if (!/^[*#\s]*Почему так/.test(t)) return false;
  const cyr = (t.match(/[А-Яа-яЁё]/g) ?? []).length;
  const lat = (t.match(/[A-Za-z]/g) ?? []).length;
  return cyr >= 4 * lat && !/^[*#\s]*Почему так[\s\S]*\bWe need\b/.test(t);
}
